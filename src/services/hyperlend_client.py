        ...
        for attempt in range(max_retries):
            try:
                ...
                if "rate limited" in error_str or "-32005" in error_str:
                    ...
                    logger.warning(f"Rate limit hit in HyperlendClient. Sleeping {delay}s before retry {attempt+1}/{max_retries}...")
                    time.sleep(delay)
                else:
                    raise e
        ...
        for asset_address in reserves_list:
            try:
                ...
                data_row = {
                    ...
                    "pool_id": asset_address, # The underlying asset is the unique key here in context of the protocol
                    "symbol": symbol,
                    "name": name,
                    "contract_address": h_token_address, # The vault/hToken address
                    ...
                    "decimals": decimals,
                    "reserve_factor": 0.0 # Not extracted in script, default
                }
                results.append(data_row)

            except Exception as e:
                logger.error(f"Error processing {asset_address} in Hyperlend: {e}")
                time.sleep(1)
